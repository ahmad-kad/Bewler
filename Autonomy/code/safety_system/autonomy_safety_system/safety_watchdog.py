...
from autonomy_interfaces.msg import SafetyStatus, SystemState as SystemStateMsg
...
class WatchdogConfiguration:
    ...
    watchdog_levels: List[WatchdogLevel] = field(default_factory=lambda: [
...
class SafetyWatchdog(Node):
    ...
    def __init__(self, config: Optional[WatchdogConfiguration] = None) -> None:
        ...
        self.logger.info('Safety Watchdog initialized with monitoring levels: '
    ...
    def _setup_publishers(self):
        ...
        self.diagnostics_pub = self.create_publisher(
            DiagnosticArray, '/safety/diagnostics', 10, callback_group=self.monitoring_group)
    ...
    def _setup_subscribers(self):
        ...
        if WatchdogLevel.HEARTBEAT in self.config.watchdog_levels:
            self.create_subscription(
                String, '/state_machine/heartbeat', self._heartbeat_callback,
                1, callback_group=self.monitoring_group)

        # State transition monitoring
        if WatchdogLevel.STATE_TRANSITIONS in self.config.watchdog_levels:
            self.create_subscription(
                SystemStateMsg, '/state_machine/current_state', self._state_callback,
                1, callback_group=self.monitoring_group)

        # Subsystem health monitoring
        if WatchdogLevel.SUBSYSTEM_HEALTH in self.config.watchdog_levels:
            self.create_subscription(
                String, '/state_machine/subsystem_status', self._subsystem_callback,
        ...
        if WatchdogLevel.SENSOR_INTEGRITY in self.config.watchdog_levels:
            self.create_subscription(
                BatteryState, '/battery/status', self._battery_callback,
                10, qos_profile=self.qos_best_effort, callback_group=self.monitoring_group)
    ...
    def _setup_timers(self):
        ...
        if WatchdogLevel.HEARTBEAT in self.config.watchdog_levels:
            self.heartbeat_timer = self.create_timer(
                self.config.heartbeat_timeout, self._check_heartbeat_timeout,
                callback_group=self.monitoring_group)

        # State transition timeout check
        if WatchdogLevel.STATE_TRANSITIONS in self.config.watchdog_levels:
            self.state_timer = self.create_timer(
                self.config.state_transition_timeout, self._check_state_timeout,
                callback_group=self.monitoring_group)

        # Subsystem health check
        if WatchdogLevel.SUBSYSTEM_HEALTH in self.config.watchdog_levels:
            self.subsystem_timer = self.create_timer(
                self.config.subsystem_health_timeout, self._check_subsystem_health,
                callback_group=self.monitoring_group)

        # Sensor integrity check
        if WatchdogLevel.SENSOR_INTEGRITY in self.config.watchdog_levels:
            self.sensor_timer = self.create_timer(
                self.config.sensor_integrity_timeout, self._check_sensor_integrity,
                callback_group=self.monitoring_group)
    ...
    def _subsystem_callback(self, msg: String):
        ...
        try:
            ...
            failed_subsystems = [
                name for name, status in subsystem_data.items()
    ...
    def _trigger_safety_violation(self, violation_id: str, severity: SafetySeverity,
    ...
    def _publish_diagnostics(self):
        ...
        elif any(v['severity'] in [SafetySeverity.CRITICAL, SafetySeverity.EMERGENCY]
        ...
        for violation_id, violation_data in self.active_violations.items():
            watchdog_status.values.append(
                {'key': f'violation_{violation_id}', 'value': violation_data['description']})
