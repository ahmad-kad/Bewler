...
def generate_launch_description():
    ...
    camera_device_arg = DeclareLaunchArgument(
        "camera_device", default_value="/dev/video0", description="Camera device path"
    )
    ...
    model_path_arg = DeclareLaunchArgument(
        "model_path", default_value="", description="Path to TFLite INT8 model file"
    )

    camera_name_arg = DeclareLaunchArgument(
        "camera_name", default_value="camera", description="Name of the camera"
    )
    ...
    frame_width_arg = DeclareLaunchArgument(
        "frame_width", default_value="640", description="Frame width for camera capture"
    )
    ...
    frame_rate_arg = DeclareLaunchArgument(
        "frame_rate", default_value="15.0", description="Frame rate for camera capture"
    )
    ...
    v4l2_camera_node = Node(
        ...
        remappings=[
            (
                "image_raw",
                f'/camera/{LaunchConfiguration("camera_name")}/image_raw/compressed',
            ),
            (
    ...
    log_launch = LogInfo(
        msg=[
            ...
            "\n",
            "  H.264 device: ",
            LaunchConfiguration("h264_device"),
            "\n",
            "  Camera name: ",
            ...
            "\n",
            "  Inference enabled: ",
            LaunchConfiguration("enable_inference"),
            "\n",
            "  Streaming enabled: ",
            LaunchConfiguration("enable_streaming"),
            "\n",
            "  Resolution: ",
