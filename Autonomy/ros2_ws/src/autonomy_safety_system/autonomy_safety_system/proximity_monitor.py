...
class ProximityMonitor(Node):
    ...
    def __init__(self):
        ...
        self.qos_reliable = QoSProfile(reliability=ReliabilityPolicy.RELIABLE, history=HistoryPolicy.KEEP_LAST, depth=5)
    ...
    def _setup_subscribers(self):
        ...
        self.create_subscription(
            LaserScan, "/scan", self._lidar_callback, self.qos_sensor, callback_group=self.create_callback_group()
        )
    ...
    def _track_sensor_aoi(self, sensor_id: str, sensor_timestamp: float):
        ...
        if current_aoi > self.aoi_config.critical_threshold:
            self.logger.warn(f"Safety sensor AoI violation: {sensor_id}={current_aoi:.3f}s", throttle_duration_sec=5.0)
    ...
    def _check_proximity(self):
        ...
        if not self._has_fresh_safety_data():
            self.logger.warn("No fresh safety sensor data available - cannot ensure safety", throttle_duration_sec=2.0)
    ...
    def _trigger_proximity_violation(self, zone: ProximityZone, reading: ProximityReading):
        ...
        severity_map = {"WARNING": "WARNING", "CRITICAL": "CRITICAL", "EMERGENCY": "EMERGENCY"}
