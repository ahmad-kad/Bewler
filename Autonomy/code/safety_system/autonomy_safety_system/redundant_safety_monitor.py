...
class RedundantSafetyMonitor(Node):
    ...
    def _setup_subscribers(self):
        ...
        self.create_subscription(
            SafetyStatus, '/state_machine/safety_status', self._primary_safety_callback,
            10, callback_group=self.monitoring_group)

        # Independent sensor monitoring
        self.create_subscription(
            Imu, '/imu/data', self._imu_safety_check,
            10, qos_profile=self.qos_best_effort, callback_group=self.validation_group)

        self.create_subscription(
            BatteryState, '/battery/status', self._battery_safety_check,
            10, qos_profile=self.qos_best_effort, callback_group=self.validation_group)

        self.create_subscription(
            Temperature, '/temperature/data', self._temperature_safety_check,
            10, qos_profile=self.qos_best_effort, callback_group=self.validation_group)

        self.create_subscription(
            Odometry, '/wheel/odom', self._odometry_safety_check,
            10, qos_profile=self.qos_best_effort, callback_group=self.validation_group)
    ...
    def _battery_safety_check(self, msg: BatteryState):
        ...
        if msg.percentage <= self.thresholds.battery_critical:
            ...
            if self.current_state == RedundantSafetyState.NORMAL:
                self._update_redundant_state(RedundantSafetyState.WARNING,
    ...
    def _trigger_redundant_violation(self, violation_id: str, description: str, context: Dict[str, Any]):
        ...
        violation_msg.is_safe = self.current_state not in [RedundantSafetyState.EMERGENCY, RedundantSafetyState.CRITICAL]
    ...
    def _update_redundant_state(self, new_state: RedundantSafetyState, reason: str):
        ...
        if new_state != self.current_state:
            self.logger.info(f'Redundant safety state changed: {self.current_state.value} -> {new_state.value} ({reason})')
    ...
    def _update_redundant_state_from_primary(self):
        ...
        if primary_level == 'EMERGENCY':
            if self.current_state != RedundantSafetyState.EMERGENCY:
                self._update_redundant_state(RedundantSafetyState.EMERGENCY,
                                           'Primary system emergency detected')
        elif primary_level == 'CRITICAL':
            if self.current_state not in [RedundantSafetyState.EMERGENCY, RedundantSafetyState.CRITICAL]:
                self._update_redundant_state(RedundantSafetyState.CRITICAL,
    ...
    def _perform_consistency_check(self):
        ...
        if not self.primary_safety_status:
            ...
            if time_since_primary > self.thresholds.communication_timeout:
                ...
                elif (primary_level in ['NORMAL', 'WARNING'] and
                      redundant_level in ['NORMAL', 'WARNING', 'MONITORING']):
    ...
    def _evaluate_redundant_safety(self):
        ...
        # Check sensor health
        unhealthy_sensors = [
            sensor for sensor, health in self.sensor_health.items()
        ...
        active_violations = len([r for r in self.consistency_history
        ...
        if active_violations > 10:  # Many recent inconsistencies
            ...
            major_issues = sum(1 for r in recent_consistency
                             if r['consistency'] in [SafetyConsistency.MAJOR_DISCREPANCY.value,
