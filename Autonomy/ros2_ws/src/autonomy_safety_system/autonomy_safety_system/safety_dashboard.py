...
class SafetyDashboard(Node):
    ...
    def _setup_subscribers(self):
        ...
        # System monitoring
        self.create_subscription(
            String, "/safety/watchdog_status", self._watchdog_status_callback, 10, callback_group=self.aggregation_group
        )

        self.create_subscription(
            String, "/safety/sensor_health", self._sensor_health_callback, 10, callback_group=self.aggregation_group
        )
    ...
    def _safety_violations_callback(self, msg: SafetyStatus):
        ...
        if not msg.is_safe:
            ...
            self._create_alert(
                ...
                context={"trigger_type": msg.trigger_type, "trigger_source": msg.trigger_source},
            )
        ...
        try:
            ...
            if emergency_data.get("emergency_active", False):
                ...
                self._update_system_health(
                    "emergency_response_coordinator", SafetySystemStatus.DEGRADED, time.time(), 0.3
                )
            else:
                self._update_system_health(
                    "emergency_response_coordinator", SafetySystemStatus.HEALTHY, time.time(), 1.0
                )
    ...
    def _create_alert(
        self, alert_id: str, level: SafetyAlertLevel, source: str, message: str, context: Dict[str, Any] = None
    ):
        ...
        alert = SafetyAlert(
            alert_id=alert_id, timestamp=time.time(), level=level, source=source, message=message, context=context or {}
        )
    ...
    def _update_system_health(
        ...
        if system_name not in self.system_health:
            self.system_health[system_name] = SystemHealthStatus(
                system_name=system_name, status=status, last_update=update_time, health_score=health_score or 0.5
            )
        else:
    ...
    def _publish_dashboard_diagnostics(self):
        ...
        for system_name, health in self.system_health.items():
            dashboard_status.values.append(
                {"key": f"{system_name}_health", "value": f"{health.status.value} ({health.health_score:.2f})"}
            )
