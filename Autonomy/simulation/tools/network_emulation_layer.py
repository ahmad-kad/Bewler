...
class NetworkEmulationLayer(Node):
    ...
    def __init__(self):
        ...
        # Publishers
        self.network_status_pub = self.create_publisher(String, "/network_emulation/status", 10)

        self.network_metrics_pub = self.create_publisher(Float32MultiArray, "/network_emulation/metrics", 10)
    ...
    def intercept_callback(self, msg):
        ...
        self.delayed_messages.append({"message": msg, "delivery_time": delivery_time, "original_time": time.time()})
    ...
    def generate_latency(self) -> float:
        ...
        base_latency = random.uniform(self.current_profile.latency_ms[0], self.current_profile.latency_ms[1])
            ...
            if self.connection_state:
                ...
                if random.random() < 0.001:  # 0.1% chance per 100ms
                    ...
                    drop_duration = random.uniform(
                        self.current_profile.drop_duration_sec[0], self.current_profile.drop_duration_sec[1]
                    )
    ...
    def get_network_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "messages_dropped": self.statistics.messages_dropped,
            "packet_loss_rate": (self.statistics.messages_dropped / max(1, self.statistics.messages_sent) * 100),
            "average_latency_ms": self.statistics.average_latency_ms,
            ...
            "uptime_percentage": (
                100.0 - (self.statistics.total_downtime_sec / max(1, time.time() - self.start_time) * 100)
            ),
        }
