...
class SafetyWatchdog(Node):
    ...
    def _setup_subscribers(self):
        ...
        if WatchdogLevel.HEARTBEAT in self.config.watchdog_levels:
            self.create_subscription(
                String, "/state_machine/heartbeat", self._heartbeat_callback, 1, callback_group=self.monitoring_group
            )
    ...
    def _setup_timers(self):
        ...
        if WatchdogLevel.HEARTBEAT in self.config.watchdog_levels:
            self.heartbeat_timer = self.create_timer(
                self.config.heartbeat_timeout, self._check_heartbeat_timeout, callback_group=self.monitoring_group
            )

        # State transition timeout check
        if WatchdogLevel.STATE_TRANSITIONS in self.config.watchdog_levels:
            self.state_timer = self.create_timer(
                self.config.state_transition_timeout, self._check_state_timeout, callback_group=self.monitoring_group
            )

        # Subsystem health check
        if WatchdogLevel.SUBSYSTEM_HEALTH in self.config.watchdog_levels:
            self.subsystem_timer = self.create_timer(
                self.config.subsystem_health_timeout, self._check_subsystem_health, callback_group=self.monitoring_group
            )

        # Sensor integrity check
        if WatchdogLevel.SENSOR_INTEGRITY in self.config.watchdog_levels:
            self.sensor_timer = self.create_timer(
                self.config.sensor_integrity_timeout, self._check_sensor_integrity, callback_group=self.monitoring_group
            )
    ...
    def _trigger_safety_violation(
        self, violation_id: str, severity: SafetySeverity, description: str, context: Dict[str, Any]
    ):
    ...
    def _execute_safety_stop(self, violation_id: str, severity: SafetySeverity, description: str):
        ...
        safety_msg.recovery_steps = ["Investigate safety violation", "Clear safety stop", "Verify system health"]
    ...
    def _publish_diagnostics(self):
        ...
        for violation_id, violation_data in self.active_violations.items():
            watchdog_status.values.append({"key": f"violation_{violation_id}", "value": violation_data["description"]})
