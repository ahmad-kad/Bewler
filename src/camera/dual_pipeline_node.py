...
class DualPipelineNode(Node):
    ...
    def __init__(self):
        ...
        # Create publishers
        self.image_pub = self.create_publisher(Image, f'/camera/{self.camera_name}/image_raw', qos)
        self.compressed_pub = self.create_publisher(Image, f'/camera/{self.camera_name}/image_raw/compressed', qos)
        self.detection_pub = self.create_publisher(Detection2DArray, f'/camera/{self.camera_name}/detections', qos)
        ...
        self.get_logger().info(f'Dual pipeline node initialized for {self.camera_device}')
    ...
    def _initialize_camera(self):
        ...
        try:
            ...
            self.get_logger().info(f"Camera initialized: {actual_width}x{actual_height} @ {self.frame_rate}fps")
    ...
    def _initialize_h264_encoder(self):
        ...
        try:
            ...
            self.get_logger().info(f"H.264 encoder initialized for device {self.h264_device}")
    ...
    def load_calibration(self):
        ...
        try:
            ...
            self.calib_data = load_calibration_file(self.camera_name, os.path.dirname(self.calibration_file))
            self.get_logger().info(f"Calibration loaded for {self.camera_name}")
        except Exception as e:
    ...
    def create_detection_msg(self, detections: List[Dict[str, Any]], timestamp) -> Detection2DArray:
